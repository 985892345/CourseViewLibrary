...
import com.ndhzs.courseviewlibrary.demo.widget.course.helper.multitouch.PointerFlag
...
import com.ndhzs.courseviewlibrary.demo.widget.course.helper.utils.VibratorUtil
...
internal class CreateAffairTouchHandler(
    ...
    private var mTouchAffairView: TouchAffairView? = null
    ...
    private fun recoverFoldState() {
        ...
        for (i in 0 until course.childCount) {
            // 判断中午和傍晚时间段是否存在 View
            val child = course.getChildAt(i)
            if (child === mTouchAffairView || child.isInvisible || child.isGone) continue
            val lp = child.layoutParams as CourseLayoutParams
            when (lp.type) {
                CourseType.TIME, CourseType.SUBSTITUTE,
                CourseType.UNKNOWN -> {}
                else -> {
                    if (course.isContainNoon(lp)) hasViewInNoon = true
                    if (course.isContainDusk(lp)) hasViewInDusk = true
                }
            }
            if (hasViewInNoon && hasViewInDusk) break
        }
        val lp = mTouchAffairView!!.layoutParams as CourseLayoutParams
        if (mIsNoonFoldedWhenLongPress && !course.isContainNoon(lp) && !hasViewInNoon) {
            when (course.getNoonRowState()) {
                RowState.UNFOLD, RowState.UNFOLD_ANIM -> course.foldNoonForce()
                else -> {}
            }
        }
        if (mIsDuskFoldedWhenLongPress && !course.isContainDusk(lp) && !hasViewInDusk) {
            when (course.getDuskRowState()) {
